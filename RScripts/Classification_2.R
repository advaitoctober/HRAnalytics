...
#Bagging
############################################
hrBAGMod = randomForest(left~.,data = hr_train, n.tree =10000, mtry = 7)
varImpPlot(hrBAGMod)
...
ggplot(errorDF, aes(x = Model_Name,group = 1)) + 
  ...
  scale_x_discrete(limits=c("Logistic regression","LDA","QDA","Single Pruned Tree","Random Forest","Bagging","Boosting"))+
